(function ($) {
    "use strict";
    var G5_Portfolio_Horizontal = window.G5_Portfolio_Horizontal || {};
    window.G5_Portfolio_Horizontal = G5_Portfolio_Horizontal;
    G5_Portfolio_Horizontal = {
        init: function () {
            $('.gf-portfolio-horizontal').each(function () {
                G5_Portfolio_Horizontal.execDots($(this));
            });
        },
        execDots: function ($portfolio_horizontal) {
            var $item_container = $('[data-items-container]', $portfolio_horizontal),
                $item_count = $('article', $item_container).length;
            if($item_count) {
                var $dot_html = '<div class="gsf-portfolio-horizontal-dots">';
                ...
                $dot_html += '</div>';
                $portfolio_horizontal.append($dot_html);
                var $dot_container = $('.gsf-portfolio-horizontal-dots', $portfolio_horizontal);
                ...
                G5_Portfolio_Horizontal.execPosition($item_container, $dot_container);
            }
        },
        execPosition: function ($item_container, $dot_container) {
            ...
            $item_container.on('owlInitialized', function () {
                if(window.matchMedia('(min-width: 1350px)').matches) {
                    G5_Portfolio_Horizontal.execPositionContent($item_container, $dot_container);
                }
            });
