...
if (!class_exists('G5P_Widget_Twitter')) {
    class G5P_Widget_Twitter extends GSF_Widget
    {
        public function __construct()
        {
            ...
            $this->widget_description = esc_html__("Display your latest tweets", 'spring-framework');
            $this->widget_name = esc_html__('G5Plus: Twitter', 'spring-framework');
            $this->settings = array(
                'fields' => array(
                    array(
                        'id'      => 'title',
                    ...
                    array(
                        ...
                        'type'    => 'text',
                        'default' => '',
                        'title'   => esc_html__('User Name', 'spring-framework')
                    ),
                    array(
                        'id'      => 'consumer_key',
                        'type'    => 'text',
                        'default' => '',
                        'title'   => esc_html__('Consumer Key', 'spring-framework')
                    ),
                    array(
                        'id'      => 'consumer_secret',
                        'type'    => 'text',
                        'default' => '',
                        'title'   => esc_html__('Consumer Secret', 'spring-framework')
                    ),
                    array(
                        'id'      => 'access_token',
                        'type'    => 'text',
                        'default' => '',
                        'title'   => esc_html__('Access Token', 'spring-framework')
                    ),
                    array(
                        'id'      => 'access_token_secret',
                        'type'    => 'text',
                        'default' => '',
                        'title'   => esc_html__('Access Token Secret', 'spring-framework')
                    ),
                    array(
                        'id'      => 'time_to_store',
                        'type'    => 'text',
                        'default' => '',
                        'title'   => esc_html__('Time To Store', 'spring-framework')
                    ),
                    array(
                        'id'      => 'total_feed',
                        'type'    => 'text',
                        'default' => '',
                        'title'   => esc_html__('Total Feed', 'spring-framework')
                    )
                )
            );
            parent::__construct();
        ...
        public function widget($args, $instance)
        {
            ...
            if ($limitToDisplay > 0) {
                ...
                <div class="widget-twitter-wrapper">
                    ...
                    <?php endfor; ?>
                    <a href="<?php echo esc_html('//twitter.com/' . $user_name); ?>" target="_blank" class="follow-us btn btn-accent"><?php esc_html_e('Follow on Twitter','spring-framework')?></a>
                </div>
