	...
	class GSF_Field_File extends GSF_Field
	{
		function enqueue() {
			...
			wp_localize_script(GSF()->assetsHandle('field_file'), 'GSF_FILE_FIELD_META', array(
				'title'   => esc_html__('Select File', 'spring-framework'),
				'button'  => esc_html__('Use these files', 'spring-framework')
			));
		}
		...
		function renderContent()
		{
			...
			$remove_text = __('Remove','spring-framework');
			$lib_filter = '';
			...
			<div class="gsf-field-file-inner" data-remove-text="<?php echo esc_attr($remove_text); ?>" <?php echo ($lib_filter); ?>>
				...
				<div class="gsf-file-add">
					<button class="button" type="button"><?php esc_html_e('+ Add File','spring-framework'); ?></button>
				</div>
			</div>
			<?php
		}
