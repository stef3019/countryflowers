	...
	class G5P_Vc_Customize
	{
		...
		public function custom_param_vc_row() {
			...
			$params = array(
				array(
					'type' => 'gsf_button_set',
					'heading' => esc_html__('Container Width', 'spring-framework'),
					'description' => esc_html__('Define the width of the container.', 'spring-framework'),
					'param_name' => 'container_width',
					'value' => array(
						esc_html__('Full', 'spring-framework') => 'full',
						esc_html__('Limit', 'spring-framework') => 'limit'
					),
					'std' => 'full',
					'admin_label' => true,
					'weight' => 99
				...
				array(
					'type' => 'gsf_button_set',
					'heading' => esc_html__('Content Width', 'spring-framework'),
					'description' => esc_html__('Define the width of the content area.', 'spring-framework'),
				...
				array(
					'type' => 'gsf_button_set',
					'heading' => esc_html__('Background Overlay', 'spring-framework'),
					'param_name' => 'bg_overlay_mode',
					'description' => esc_html__('Specify overlay mode for the background.', 'spring-framework'),
					'value' => array(
						esc_html__('Hide', 'spring-framework') => '',
						esc_html__('Color', 'spring-framework') => 'color',
				...
				array(
					'type' => 'colorpicker',
					'heading' => esc_html__('Overlay color', 'spring-framework'),
					'param_name' => 'bg_overlay_color',
					'description' => esc_html__('Specify an overlay color for the background.', 'spring-framework'),
					'dependency' => array('element' => 'bg_overlay_mode', 'value' => 'color'),
					'weight' => ($background_overlay_index - 1)
				),
				array(
					'type' => 'attach_image',
					'heading' => esc_html__('Overlay Image', 'spring-framework'),
					'param_name' => 'bg_overlay_image',
					'description' => esc_html__('Specify an overlay image for the background.', 'spring-framework'),
					'dependency' => array('element' => 'bg_overlay_mode', 'value' => 'image'),
					'weight' => ($background_overlay_index - 2)
				),
				array(
					'type' => 'gsf_slider',
					'heading' => esc_html__('Overlay Opacity', 'spring-framework'),
					...
					'description' => esc_html__('Specify overlay opacity for the background.', 'spring-framework'),
					'dependency' => array('element' => 'bg_overlay_mode', 'value' => 'image'),
                ...
                if ( '' !== $skin_id) {
                    ...
                    if (function_exists('Spring_Plant')) {
                        ...
                        if (defined('CSS_DEBUG') && CSS_DEBUG) {
                            wp_enqueue_style(Spring_Plant()->helper()->assetsHandle("skin-{$skin_id}"), admin_url('admin-ajax.php') . '?action=gsf_dev_less_skin_to_css&skin_id=' . $skin_id, array(), false);
                        } else {
                            do_action('gsf_before_enqueue_skin_css',$skin_id);
                            wp_enqueue_style(Spring_Plant()->helper()->assetsHandle("skin-{$skin_id}"), Spring_Plant()->helper()->getAssetUrl("assets/skin/{$skin_id}.min.css"));
                        }
                    }
                }
            }
...
CSS;
		...
		{
			$colors = array(
				esc_html__('Accent Color', 'spring-framework') => 'primary'
			);
			$param_color = WPBMap::getParam('vc_tta_tabs', 'color');
			$param_color['value'] = array_merge($colors, $param_color['value']);
			$param_color['heading'] = esc_html__('Color', 'spring-framework');
