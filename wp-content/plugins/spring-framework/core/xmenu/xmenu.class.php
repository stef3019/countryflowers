...
if (!class_exists('G5P_Core_XMenu')) {
	final class G5P_Core_XMenu
	{
		...
		public function register_post_type_mega_menu() {
			...
			$post_type_name = esc_html__('Mega Menu', 'spring-framework');
			$singular_name = esc_html__('Mega Menu', 'spring-framework');
			...
			$args = array(
				...
				'labels'             => array(
					...
					'add_new_item'          => sprintf(__('Add New %s', 'spring-framework'), $singular_name),
					'edit_item'             => sprintf(__('Edit %s', 'spring-framework'), $singular_name),
					'new_item'              => sprintf(__('New %s', 'spring-framework'), $singular_name),
					'view_item'             => sprintf(__('View %s', 'spring-framework'), $singular_name),
					'search_items'          => sprintf(__('Search %s', 'spring-framework'), $post_type_name),
					'not_found'             => sprintf(__('No %s found.', 'spring-framework'), strtolower($post_type_name)),
					'not_found_in_trash'    => sprintf(__('No %s found in Trash.', 'spring-framework'), strtolower($post_type_name)),
					'all_items'             => sprintf(__('All %s', 'spring-framework'), $post_type_name),
					'archives'              => sprintf(__('%s Archives', 'spring-framework'), $post_type_name),
					'insert_into_item'      => sprintf(__('Insert into %s', 'spring-framework'), strtolower($singular_name)),
					'uploaded_to_this_item' => sprintf(__('Uploaded to this %s', 'spring-framework'), strtolower($singular_name)),
					'filter_items_list'     => sprintf(__('Filter %s list', 'spring-framework'), strtolower($post_type_name)),
					'items_list_navigation' => sprintf(__('%s list navigation', 'spring-framework'), $post_type_name),
					'items_list'            => sprintf(__('%s list', 'spring-framework'), $post_type_name),
				)
			);
