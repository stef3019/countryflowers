...
(function ($) {
            ...
            $('.isotope', $wrapper).each(function () {
                ...
                $this.imagesLoaded({background: true}, function () {
                    ...
                    if ((typeof (config.masonry) !== 'undefined')
                        ...
                        $this.append('<article class="gsf-column-base"></article>');
                    }
        ...
        get_multiplier_width: function ($item) {
            ...
            if ($item.is('[class]')) {
                var _class = $item.attr('class'),
                    multiplier_mb_w = _class.match(/col-(\d{0,2})/),
                    ...
                    multiplier_xl_w = _class.match(/col-xl-(\d{0,2})/);
                ...
                if (window.matchMedia('(min-width: 1200px)').matches) {
                    if (_class.match(/col-xl-12-5/)) {
                        multiplier_w = 12;
                    } else if (multiplier_xl_w !== null) {
                        multiplier_w = multiplier_xl_w[1] * 5;
                    }
                }
            }
            return multiplier_w;
    ...
    G5_Core.pagination_ajax = {
        ajax: false,
        laddaButton: null,
        prefix: 'spring_plant_ajax_paginate_',
        ...
        loadPosts: function (settingId, target) {
            if ($(target).hasClass('active') || $(target).hasClass('disable') || $(target).parent().hasClass('active') || $(target).hasClass('gsf-dropdown-toggle')) return;
            ...
            if (_data['type'] === 'paging') {
                ...
                if (pagination === _that.paging.loadMore ||  pagination === _that.paging.infiniteScroll) {
                    _data.query['index'] = $wrapper.find(_data.settings['itemSelector']).not('.gsf-column-base').length;
                }
    ...
    G5_Core.header = {
        ...
        events: function () {
            var _that = this,
                nav_time_out = null,
                canvas_time_out = null,
                popup_canvas_menu = $('#popup-canvas-menu'),
                menu_icon = $('.header-inner .gf-menu-canvas');
            $window.on('resize', function () {
            ...
            $window.on('click', function (event) {
                ...
                if ($(event.target).closest('.search-ajax-wrap').length === 0) {
                    $('.search-ajax-wrap [data-search-ajax-control="result"]').removeClass('in').html();
                }
            });
            $(document).on('click', '.gf-menu-canvas', function () {
                if(popup_canvas_menu.length === 0) return;
                if (canvas_time_out !== null) {
                    clearTimeout(canvas_time_out);
                }
                popup_canvas_menu.find('.primary-menu').perfectScrollbar({
                    wheelSpeed: 0.5,
                    suppressScrollX: true
                });
                canvas_time_out = setTimeout(function () {
                    if(popup_canvas_menu.hasClass('show')) {
                        popup_canvas_menu.removeClass('show');
                        menu_icon.removeClass('hide');
                    } else {
                        popup_canvas_menu.addClass('show');
