...
$product_cate_filter = Spring_Plant()->options()->get_product_cate_filter();
$query_args = $settings = null;
if('' !== $product_cate_filter) {
    $settings['category_filter_enable'] = true;
    $settings['category_filter_align'] = $product_cate_filter;
...
if ( isset( $_REQUEST['shop_load_type'] ) ) {
    if ( 'ajax' === $_REQUEST['shop_load_type'] ) {
        Spring_Plant()->helper()->getTemplate('woocommerce/archive-product-ajax', array( 'query_args'=>$query_args, 'settings'=>$settings) );
        return;
    }
}
