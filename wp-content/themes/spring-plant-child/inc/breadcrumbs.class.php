...
if (!class_exists('Spring_Plant_Inc_Breadcrumbs')) {
	class Spring_Plant_Inc_Breadcrumbs
	{
		...
		public function init()
		{
			...
			$default = array(
				...
				'show_terms' => false,
				'home_label' => esc_html__('Home', 'spring-plant'),
				'blog_label' => esc_html__('Blog', 'spring-plant'),
				'tag_archive_prefix' => esc_html__('Tag:', 'spring-plant'),
				'search_prefix' => esc_html__('Search:', 'spring-plant'),
				'error_prefix' => esc_html__('Page Not Found', 'spring-plant')
			);

			$args = apply_filters('spring_plant_breadcrumbs_options', $default);
			...
			$this->home_label = isset($default['home_label']) ? $default['home_label'] : esc_html__('Home', 'spring-plant');
			$this->blog_label = isset($default['blog_label']) ? $default['blog_label'] : esc_html__('Blog', 'spring-plant');
			$this->tag_archive_prefix = isset($default['tag_archive_prefix']) ? $default['tag_archive_prefix'] : esc_html__('Tag:', 'spring-plant');
			$this->search_prefix = isset($default['search_prefix']) ? $default['search_prefix'] : esc_html__('Search:', 'spring-plant');
			$this->error_prefix = isset($default['error_prefix']) ? $default['error_prefix'] : esc_html__('Page Not Found', 'spring-plant');
		...
		{
			if ($this->html_markup) {
			    $class = apply_filters('gf-breadcrumbs-class', array('gf-breadcrumbs', 'd-flex'));
				$this->html_markup = '<ul class="' . join(' ', $class) . '">' . $this->html_markup . '</ul>';
			}
		...
		private function get_breadcrumb_leaf_markup($object_type = '') {
			...
			return '<li itemprop="itemListElement" itemscope itemtype="https://schema.org/ListItem" class="breadcrumb-leaf cust"><span itemprop="name">' . $title . '</span></li><meta itemprop="position" content="2" />';
		}
		...
		private function get_single_breadcrumb_markup($title, $link = '', $separator = true, $micro_data = true)
		{
			...
			if ($micro_data) {
				$micro_data_item_scope = 'itemscope itemtype="//schema.org/BreadcrumbList"';
				$micro_data_url = 'itemprop="item"';
				$micro_data_title = 'itemprop="name"';
				
			}
			$micro_data_item_sub_scope = 'itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem"';
			if ($title != 'Shop') {

				//$breadcrumb_content = '<span ' . $micro_data_title . '>' . $title . '</span>  <meta itemprop="position" content="1" />';
				if ($link) {
					$breadcrumb_content = '<a ' . $micro_data_url . ' href="' . esc_url($link) . '">' . $breadcrumb_content . '</a><meta itemprop="position" content="1" />';
				}
				...
				$output = '<li ' . $micro_data_item_scope . '><span ' .$micro_data_item_sub_scope.'>'. $breadcrumb_content . '</span></li><meta itemprop="position" content="2" />' . $separator_markup;
