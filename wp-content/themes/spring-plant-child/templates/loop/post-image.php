 ...
 * @var $class
 */
$image_sizes =  Spring_Plant()->blog()->get_image_sizes();
...
<div class="<?php echo esc_attr($class) ?>">
     ...
     **/
    do_action('spring_plant_before_post_image', array(
        'post_id'    => $post_id,
        'image_id'   => $image_id,
    ...
    if ($lazy_load_images === 'on' && isset($image['width']) && !empty($image['width']) && isset($image['height']) && !empty($image['height'])) {
        ...
        $image_lazy = Spring_Plant()->image_resize()->resize(array(
            'image_id' => $image_id,
...
CSS;
                    Spring_Plant()->custom_css()->addCss($custom_css,"thumbnail-size-{$image_size}");
                }
            }
...
CSS;
                    Spring_Plant()->custom_css()->addCss($custom_css,"thumbnail-size-{$image_ratio}");
                }
            }
        }
	...
	<?php else: ?>
		...
		<?php if ($display_permalink) : ?>
			<a class="entry-thumbnail-overlay" href="<?php echo esc_url( get_permalink($post_id));?>" title="<?php the_title_attribute(array('post' => $post_id)) ?>">
